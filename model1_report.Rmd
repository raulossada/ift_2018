...
model1 <- function(t, state, parameters){
  with( as.list( c(state, parameters) ), {
    ...
    dZ  <- r_Z*Z*(1-Z/K_z) + (1-alpha)*g*(qq*pp*D + mu_Pz)*Pz - s*Z*P - i*Z*C;
    dP  <- r_P*P*(1-P/K_P) - theta*s*Z*P - (pp)*P*D;
    dPz <- r_Pz*Pz*(1-Pz/K_Pz) + theta*s*Z*P - (pp)*Pz*D;
    dD  <- delta*( (pp)*P + (pp)*Pz )*D - mu_D*D;
    dC  <- k1*i*Z*C + k2*alpha*g*(qq*pp*D + mu_Pz)*Pz*C - mu_C*C;
